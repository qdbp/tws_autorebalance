...
def test_adjust_composition_simple() -> None:
    ...
    with pytest.raises(ValueError):
        ...
        current.ratchet_toward(target, {A: 14})

    with pytest.raises(ValueError):
        # requires normalized percentages
        current.ratchet_toward(target, {A: -0.1})

    with pytest.raises(ValueError):
        # has to be in composition
        current.ratchet_toward(target, {X: 0.2})

    # A moves away from goal: do nothing
    new = current.ratchet_toward(target, {A: 0.16})
    assert new == current
    ...
    new = current.ratchet_toward(target, {A: 0.14})
    ...
    new = current.ratchet_toward(target, {B: 0.06})
...
def test_adjust_composition_compound() -> None:
    ...
    new = current.ratchet_toward(target, {A: 0.14, B: 0.04, C: 0.01, D: 0.78})
    assert round(new[A], 4) == 0.1400
    ...
    #   subtracted from A
    # expect: { A: 0.13, B: 0.05, C: 0.02, D: 0.80 }
    new = current.ratchet_toward(target, {A: 0.14, B: 0.04, C: 0.02, D: 0.78})
...
def test_extreme_values() -> None:
    ...
    new = current.ratchet_toward(target, {A: 1.0, B: 0.0, C: 1.0, D: 0.0})

    assert round(new[A], 4) == 0.0500
